...

{
  ...
  while (1)
  {
	    ...
	    Buttons_Process(&controller);

	    if(HAL_GetTick() - sensor_timer > 500)
	    {
	        sensor_timer = HAL_GetTick();

	        Controller_UpdateSensors(&controller);
	        Controller_ProcessLogic(&controller);
	        Controller_ApplyOutputs(&controller);
	    }
	    ...
	    if(HAL_GetTick() - lcd_timer > 500)
	    {
	        ...
	        char buf[16];
	        LCD_SetCursor(0,0);
	        sprintf(buf,"T:%2.1f SP:%2.1f",
	        controller.temperature,
	        controller.temp_setpoint);
